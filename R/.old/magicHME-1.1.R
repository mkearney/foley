...
### Based on your code, it appears that you have created an Rstats folder. If that's
...
###   like to convert and applying a function to each of them. There are a few
###   different "apply"-type of functions. Really, they're all just different
###   versions of `lapply`, which is short for "list apply". In other words,
###   you can take a list of things and apply a function to each one of those things
###   using `lapply`.
### In this case, since we're dealing with data frames, and since with data frames
###   each column is actually just a special case of a list, we can specify those
###   four columns (to get the equivalent of a list of objects):
Q83 <- Magic.csv[, c("Q83_1", "Q83_2", "Q83_3", "Q83_4")]

### And then use lapply to apply the likert2numeric function to each element (in
###   this case, column) of the list, which we've named Q83.
c_scale <- lapply(Q83, likert2numeric)

### Bingo bango, now convert the list returned and stored in c_scale into a data
###   frame and you've simplified the process in your original script.
c_scale <- data.frame(c_scale)

### Another lapply function is `apply`, which is designed for matrices/data frames.
###   The apply function is different from lapply because it expects you to
###   specify either rows (1) or columns (2). So, if we want to apply a function to
###   each column of Q83, then we'd give `apply` the data frame, Q83, the number 2,
###   indicating columns, and the function we'd like applied, likert2numeric.
